...
| 05/06/2026 | v0.1.0-piloto | `app/api/mobile/equipment/lookup/route.ts`, `app/api/mobile/equipment/route.ts`, `lib/server-storage.ts`, `docs/AUDITORIA_SISTEMA.md` | `npm run type-check`, `npm run build` | SUCESSO | Correcao tenant operacional: lookup valida Company por porta/token mas busca frota por `company.tenantId`, nunca por `company.id`; logs incluem companyId, companyTenantId, lookupTenantId e fleetCode. | **APROVADO** |
| 06/06/2026 | v0.1.0-piloto | `app/api/mobile/equipment/lookup/route.ts`, `app/api/mobile/equipment/route.ts`, `lib/server-storage.ts`, `docs/AUDITORIA_SISTEMA.md` | `npm run type-check`, `npm run build` | SUCESSO | Reforco do isolamento de tenantId: lookup e persistencia mobile agora ignoram `company.id` e usam exclusivamente `company.tenantId`. Melhoria nos logs de auditoria mobile. | **APROVADO** |
| 06/06/2026 | v0.1.0-piloto | `components/mapa/full-map-enterprise.tsx`, `app/mapa-operacional/page.tsx`, `lib/server-storage.ts`, `lib/types/index.ts`, `docs/AUDITORIA_SISTEMA.md` | `npm run type-check`, `npm run build`, `GET /api/equipamentos/status` | SUCESSO | Mapa Operacional passa a consumir somente o array real de `/api/equipamentos/status`; sidebar, markers e KPIs usam a mesma fonte, sem mock de frota/alarme. | **APROVADO** |
| 06/06/2026 | v0.1.0-piloto | `components/mapa/full-map-enterprise.tsx`, `lib/types/index.ts`, `docs/AUDITORIA_SISTEMA.md` | `npm run type-check` (SUCESSO, 0 erros), `npm run build` (compilacao pendente — timeout de sandbox, nao erro de codigo) | SUCESSO | C3 — Popup Operacional: popup consume objeto real do endpoint sem mock; campos ausentes exibem "Nao informado"; badges de status; alertas GPS/heartbeat > 120s; indicador GPS recente < 60s; logs [map-popup]; secoes Telemetria, Operacao, Horimetro, Parada. Tipos opcionais adicionados: `registration`, `hourmeterInitial`, `hourmeterFinal`, `stopCode`. | **APROVADO** |
| 06/06/2026 | v0.1.0-piloto | `lib/types/index.ts`, `app/api/mobile/events/batch/route.ts`, `components/mapa/full-map-enterprise.tsx`, `app/mapa-operacional/page.tsx` | `npm run type-check` (SUCESSO, 0 erros) | SUCESSO | C3 completo: clique sidebar->flyTo(zoom 17)+openPopup automatico; highlight de card selecionado; aviso inline para sem-GPS; MapController com useMap()+markerRefs via eventHandlers add/remove; batch events enriquecido com JOURNEY_START/LOCATION/HEARTBEAT/FSM_TRANSITION/WORK_STATUS/STOP_REASON/JOURNEY_END; tipo EquipmentLiveState +13 campos canonicos; popup usa campos canonicos com fallback para legado. | **APROVADO** |
| 06/06/2026 | v0.1.0-piloto | `app/monitoramento/conectividade/page.tsx` | `npm run type-check` (SUCESSO, 0 erros) | SUCESSO | Conectividade: removidos todos os mocks; KPIs (online, offline, ultimo pacote, tempo medio off, sem GPS, falhas) calculados de /api/equipamentos/status; status por faixa (OK/ATENCAO/CRITICO/SEVERO) com barras proporcionais ao total real; grafico exibe empty state "Historico insuficiente"; tabela lista frotas reais com ultima sinal, GPS, status; drawer mostra campos canOnicos do live-state; logs [connectivity]; polling 30s. | **APROVADO** |
---
