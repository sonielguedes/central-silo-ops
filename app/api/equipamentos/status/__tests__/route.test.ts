/**
 * Testes HOTFIX 6.7C.1 — /api/equipamentos/status com parada resolvida
 *
 * Casos obrigatorios (10):
 *  1. Nao retorna "NÃO INFORMADO" / "Nao informado" quando nao ha parada ativa
...
// ── Caso 1: sem "NÃO INFORMADO" quando nao ha parada ─────────────────────────
test('Caso 1 -- nao retorna "Nao informado" no stop.label quando nao ha parada ativa', async () => {
  ...
  expect(stop.label).not.toMatch(/NÃO/i);
  expect(stop.label).toBeTruthy();
});

// ── Caso 2: OPERANDO -> SEM_PARADA_ATIVA ─────────────────────────────────────
...
// ── Caso 3: PARADO sem motivo -> AGUARDANDO_APONTAMENTO ──────────────────────
...
// ── Caso 4: evento com stopCode retorna PARADA_APONTADA ──────────────────────
test('Caso 4 -- evento com stopCode/stopReason retorna PARADA_APONTADA', async () => {
  const live = makeLive({ status: 'PARADO', stopCode: undefined, stopDescription: undefined });
  const ev = makeEvent({ payload: { stopCode: 'PAR-01', stopDescription: 'Chuva forte' } });
  setup([live], [ev]);
  const [item] = await getJson(makeRequest());
  const stop = item.stop as Record<string, unknown>;
  expect(stop.state).toBe('PARADA_APONTADA');
  expect(stop.code).toBe('PAR-01');
  expect(stop.reason).toBe('Chuva forte');
  expect(stop.hasActiveStop).toBe(true);
});

// ── Caso 5: PARADA_APONTADA sem codigo -> PARADA_INCONSISTENTE ───────────────
...
// ── Caso 6: frota 2026 continua aparecendo ───────────────────────────────────
...
// ── Caso 7: GPS / heartbeat / horimetro continuam ────────────────────────────
...
// ── Caso 8: operador / matricula / operacao / implemento continuam ────────────
