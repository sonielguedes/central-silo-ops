...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const item = IntegrationJobStorage.retry(tenant.tenantId, params.id, getActor(req));
  if (!item) return fail(400, 'Retry permitido apenas para jobs FAILED ou CANCELED');
