  ...
  return <div className="min-h-screen bg-background"><Sidebar /><div className="lg:pl-64"><Header /><main className="mx-auto max-w-7xl space-y-5 p-6">
    ...
    {sheet && <>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card title="Jornada" icon={<Clock size={15}/>}><Row label="Início" value={dateTime(sheet.startedAt)}/><Row label="Fim" value={dateTime(sheet.endedAt)}/><Row label="Duração" value={duration(sheet.durationMinutes, !sheet.endedAt)}/><Row label="H. Inicial" value={hm(sheet.hourmeterStart)}/><Row label="H. Atual" value={hm(sheet.hourmeterCurrent)}/><Row label="H. Final" value={hm(sheet.hourmeterEnd)}/><Row label="Total Horas" value={hm(sheet.totalHourmeter)}/></Card>
      </div>
      ...
      {sheet.inconsistencies.length > 0 && <Card title="Alertas de auditoria" icon={<Activity size={15}/>}>{sheet.inconsistencies.map(item => <p key={item} className="text-xs text-amber-300">{item}</p>)}</Card>}
