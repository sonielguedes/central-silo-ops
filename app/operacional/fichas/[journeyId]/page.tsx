...
function JourneySheetPage() {
  ...
  return <div className="min-h-screen bg-background"><Sidebar /><div className="lg:pl-64"><Header /><main className="mx-auto max-w-7xl space-y-5 p-6">
    ...
    {sheet && <>
      ...
      <Card title="Eventos principais" icon={<Activity size={15}/>}>{sheet.events.length === 0 ? <p className="text-xs text-muted-foreground">Sem eventos operacionais.</p> : sheet.events.map((event, index) => <div key={`${event.timestamp}-${index}`} className="flex items-center justify-between border-b border-white/5 py-2 text-xs"><span className="font-bold text-white">{event.label}</span><span className="text-muted-foreground">{dateTime(event.timestamp)}</span></div>)}</Card>
    </>}
  </main></div></div>;
}
