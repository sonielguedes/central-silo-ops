...
const heartbeatRoute = () => loadRoute('api/mobile/heartbeat/route.js');
...
const superHeaders = {
  'x-silo-user-id': 'user-super',
  'x-silo-user-name': 'Super Admin',
  'x-silo-user-email': 'admin@siloops.com.br',
  'x-silo-user-role': 'SUPER_ADMIN',
  'x-silo-tenant': 'silo-ops-001',
};
...
before(async () => {
  ...
  const aRes = await mobileEquipmentRoute().POST(jsonReq('/api/mobile/equipment', {
    method: 'POST',
    headers: { 'x-company-token': 'CMP-SILO-OPS-001' },
    body: {
      equipmentId: 'eq-a',
      fleetCode: '1001',
      code: '1001',
...
test('tenant A does not see tenant B equipment/status', async () => {
  const resA = await statusRoute().GET(jsonReq('/api/equipamentos/status', { headers: { 'x-silo-tenant': 'silo-ops-001' } }));
  ...
  const resB = await statusRoute().GET(jsonReq('/api/equipamentos/status', { headers: { 'x-silo-tenant': 'tenant-b' } }));
...
test('tenant trail, ficha and reports stay isolated', async () => {
  const trailA = await trailRoute().GET(jsonReq('/api/equipamentos/trail?fleetCode=1001', { headers: { 'x-silo-tenant': 'silo-ops-001' } }));
  const trailABody = await asJson(trailA);
  assert.equal(trailABody.fleetCode, '1001');
  assert.ok(Array.isArray(trailABody.points));

  const fichaA = await fichaRoute().GET(jsonReq('/api/ficha-operador?fleetCode=1001', { headers: { 'x-silo-tenant': 'silo-ops-001' } }));
  assert.equal(fichaA.status, 200);
  const fichaABody = await asJson(fichaA);
  assert.equal(fichaABody.fleetCode, '1001');

  const fichaB = await fichaRoute().GET(jsonReq('/api/ficha-operador?fleetCode=2001', { headers: { 'x-silo-tenant': 'tenant-b' } }));
  assert.equal(fichaB.status, 200);
  const fichaBBody = await asJson(fichaB);
  assert.equal(fichaBBody.fleetCode, '2001');

  const relA = await relEffRoute().GET(jsonReq('/api/relatorios/eficiencia-operacional', { headers: { 'x-silo-tenant': 'silo-ops-001' } }));
  ...
  const relB = await relTempoRoute().GET(jsonReq('/api/relatorios/tempo-operacional', { headers: { 'x-silo-tenant': 'tenant-b' } }));
