        ...
        {fleet.filter(m => m.pos !== null).map(machine => {
          ...
          return (
            <Marker key={machine.id} position={machine.pos!}
              ...
              eventHandlers={{ popupopen: () => logPopupOpen(machine) }}>
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-[#2d3647]/60">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-base font-black italic tracking-tighter leading-none uppercase truncate">
              {machine.code}
            </span>
      ...
      <div className="px-4 py-3 flex flex-col gap-3">
        ...
        <PSection label="Operacao">
          <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
            ...
            <PField icon={<Hash size={11} />} label="Matricula"  value={formatValue(machine.registration)} />
            <PField icon={<Zap  size={11} />} label="Operacao"   value={machine.displayOperation} />
          </div>
        </PSection>
        <PSection label="Horimetro">
          <div className="grid grid-cols-3 gap-x-4 gap-y-2.5">
            <PField icon={<Clock size={11} />} label="Inicial" value={formatHourmeter(machine.hourmeterInitial)} />
            <PField icon={<Clock size={11} />} label="Atual"   value={formatHourmeter(machine.hourmeter)} />
            <PField icon={<Clock size={11} />} label="Final"   value={formatHourmeter(machine.hourmeterFinal)} />
          </div>
        </PSection>
        <PSection label="Parada">
          <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
            <PField icon={<PauseCircle size={11} />} label="Motivo" value={formatValue(machine.stopReason)} />
